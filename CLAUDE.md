# Brett Snyder Portfolio — Claude Code Instructions

## Reference Documents
Read before every build task — live source of truth:
- **[docs/northstar.md](docs/northstar.md)** — mission, audience, non-negotiables
- **[docs/build-plan.md](docs/build-plan.md)** — stack, features, content workflow
...
- User describes a new task → suggest `/start [task]` before proceeding
- Scope has expanded or work has gone long → suggest `/checkpoint`
- Feature declared done → suggest `/review-page [feature]` before moving on
- Error appears → suggest `/debug [error]` for systematic diagnosis
