...
// Generate latest project data for the homepage
async function generateLatestProjectsData(projectIds = []) {
  if (projectIds.length === 0) {
    projectIds = await discoverProjects();
  }
  ...
  for (const projectId of projectIds) {
    try {
      const dataPath = path.join(PROJECTS_DIR, projectId, 'project.json');

      if (fs.existsSync(dataPath)) {
        const projectData = JSON.parse(await fs.promises.readFile(dataPath, 'utf-8'));
        projects.push({
          id: projectId,
          title: projectData.title || 'Untitled',
          excerpt: projectData.excerpt || projectData.description || '',
          category: projectData.category || 'Project',
          publishDate: projectData.publishDate || new Date().toISOString().split('T')[0],
          technologies: projectData.technologies || []
        });
      }
    } catch (error) {
      console.warn(`  ⚠️  Failed to load project ${projectId}:`, error.message);
    }
  }

  projects.sort((a, b) => new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime());
...
export const latestProjects: LatestProject[] = ${JSON.stringify(projects.slice(0, 2), null, 2)};
`;

  const outputPath = path.join(process.cwd(), 'src/lib/generated/latestProjects.ts');
  await fs.promises.writeFile(outputPath, fileContent, 'utf-8');
}
...
async function runConverter(options = {}) {
  ...
  try {
    ...
    // Step 7: Generate latest content for homepage
    await generateLatestBlogsData(blogIds);
    await generateLatestProjectsData(projectIds);
